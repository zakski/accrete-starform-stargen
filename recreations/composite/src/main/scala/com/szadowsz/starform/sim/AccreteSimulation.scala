...
/**
  ...
  */
abstract class AccreteSimulation[S <: Star,R <: SimulationStats[R], P <: Planetismal, X <: StarSystem[S,R,P]](profile : SimulationProfile) extends Simulation[S,R,P,X] with AccreteSimConstants {
  ...
  /**
    ...
    */
  final override def generateSystem(seedOpt: Option[Long] = None): X = {
    logger.debug("Initialising Statistics Recorder")
    stats = initStats()
