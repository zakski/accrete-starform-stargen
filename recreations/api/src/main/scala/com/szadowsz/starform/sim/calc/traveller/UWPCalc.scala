package com.szadowsz.starform.sim.calc.traveller
...
trait UWPCalc {
  ...
  protected def calcUWPAtmos(surfacePressure: Double): Char = {
    pressure.find{ case (p,_) => surfacePressure < p}.map(_._2).getOrElse('D')
  }
  ...
  protected def calcUWPHydrographics(hydrosphere: Double, iceCover: Double): Char = {
    ...
    hydrographics.find(lvl <= _)
      .map(bin => hydrographics.indexOf(bin).toString.head)
      .getOrElse('A')
  }
}
