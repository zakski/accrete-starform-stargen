 ...
 *
 * @note dustDensity function was adjusted to be Fogg's Version, if running dole's simulation ensure that values
 *       representing earth's star are used to maintain parity.
 * @author Zakski : 03/06/2015.
 *
 * @param gCalc Calculation about the limits of a planetismal's ability to gravitationally attract dust and gas.
 * @param aConst Calculation Constants, generally provided by an instance of AccreteSimConstants to provide some user control
 */
case class AccreteCalc(gCalc : GravityCalc, aConst: AccreteConstants) {
  
  /**
   * Method to calculate outer dust band edge. The Formula that this comes from is unknown, it seems to be introduced in
   * one of the early accrete versions, but I cannot see any reference to it in the research papers. In the base
   * process, this might as well just return 200 since the solar mass is assumed to be 1.
   *
   * @note code derived from Folkins' approach to the problem simply uses a modified outer limit that we expect planets
   *       to form at.
   *
  ...
  /**
   * Method to calculate dust cloud density at a given radius. Original Formula taken from "III. Experimental Simulation
   * section a) Initial Conditions in the Cloud" in "Formation of Planetary Systems by Aggregation: A Computer
   * Simulation". In the case of simulations (not Dole's) that use a central star that is not identical to ours, we
   * alter the Density by its mass.
   *
  ...
  def dustAndGasDensity(dustDensity: Double, criticalMass: Double, mass: Double): Double = {
    ...
    def bandVolume(mass: Double, axis: Double, ecc: Double,
      ...
      val relativeMass = gCalc.muMassFunction(mass)
  ...
  /**
   * Method to calculate whether the Accretion Process for a protoplanet should continue, see "III. Experimental
   * Simulation section c) Aggregation"  from "Formation of Planetary Systems by Aggregation: A Computer Simulation".
   *
