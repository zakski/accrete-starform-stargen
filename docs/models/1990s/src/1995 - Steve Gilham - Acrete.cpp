...
{  
    U4 temp;
    cout << "Enter seed value (0 default)";
    cin >> temp;
...
Sun::Sun()
{  
    cout << "Enter mass of star (Sun=1), range 0.1 to 10\n";
    cin >> m;
    if(m < 0.1 || m > 10) m = 1.0;
    luminosity = m >= 0.4 ?
                 m*m*m*m :
...
    cout << "Enter relative gas density (Dole=1)\n";
    cin >> gas;
...
{  
    cout << "#\tOrbit\tTotalMass\tRockyMass\tecc\n";
    cout << i << "\t";  
    cout << semiMajorAxis << "\t";
    cout << totalMass/0.03 << "\t";
    cout << rockyMass/0.03 << "\t";
...
    cout << "\tyear\ttemp(C)\tincl\n\t";
    cout << safesqrt(semiMajorAxis/s.mass())*semiMajorAxis << "\t";
    cout << 288*safesqrt(safesqrt(s.flux())/semiMajorAxis)-273 << "\t";
    cout << 180*(1-safepow(r->next(),(2.0/9.0))) << "\n";

    cout << "\tradius(km)\tg(m/s^2)\n\t";
    R8 radius;
