...
/**
  ...
  */
trait StarformAccrCalc extends AccreteCalc with StarRequired {

  override val pCalc : StarformPlanCalc
  ...
  override def dustDensity(radius: Double): Double = {
    (aConst.DUST_DENSITY_COEFF * Math.sqrt(star.mass)) * Math.exp(-aConst.ALPHA * Math.pow(radius, 1.0 / aConst.N))
  }
}
