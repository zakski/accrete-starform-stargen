...
import com.szadowsz.stargen.base.accrete.system.bodies.{DustBand, Planetismal, ProtoPlanet}
...
import com.szadowsz.stargen.base.accrete.util.RandUtil
...
/**
  ...
  */
abstract class AccreteSimulation[S <: SimulationStats[S], P <: Planetismal, R <: AbstractSystem[S,P]] {
  ...
  /**
    ...
    */
  protected val iStrat: InsertionStrat
  ...
  /**
    ...
    */
  final protected  def createProtoplanet(mass: Double, axis: Double, ecc: Double): ProtoPlanet = new ProtoPlanet(this, mass, axis, ecc)
  ...
  final protected def mergeTwoPlanets(planet: ProtoPlanet, newcomer: ProtoPlanet): Unit = {
    ...
    val new_ecc: Double = coalesceEccentricity(planet.mass, planet.axis, planet.ecc, newcomer.mass, newcomer.axis, newcomer.ecc, new_axis)
  ...
  final protected def accrete(): Unit = {
    ...
    planetismals = Nil
    dust = List(DustBand(0.0, outerDustLimit(1.0))) // TODO outerDustLimit function goes against the spirit of the base sim and needs to be refactored.

    while (isDustAvailable(INNERMOST_PLANET, OUTERMOST_PLANET)) {
      val proto: ProtoPlanet = createProtoplanet(PROTOPLANET_MASS, iStrat.semiMajorAxis(rand, stats.injectedNuclei, this, dust), eccentricity(rand))
  ...
  final def generateSystem(seedOpt: Option[Long] = None): R = {
    ...
    val seed = seedOpt.getOrElse(RandUtil.extractSeed(rand))
